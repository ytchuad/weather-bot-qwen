"""Hourly append-and-close storage for one-minute market snapshots."""
...
class MarketSnapshotStore:
    ...
    @staticmethod
    def _hour_directory(root: Path, snapshot: Mapping[str, Any]) -> Path:
        ...
        return root / f"date={event_date}" / f"hour={local.hour:02d}"
    ...
    def close_due(self, now: datetime | None = None) -> dict[str, int]:
        now_local = (now or datetime.now(timezone.utc)).astimezone(HKT)
        closed = failed = 0
        with self._lock:
            for info in self.scan():
                ...
                if not hour_match or not date_match:
                    continue
                try:
                    partition_hour = datetime.strptime(
                        f"{date_match.group(1)} {hour_match.group(1)}", "%Y-%m-%d %H"
                    ).replace(tzinfo=HKT)
                except ValueError:
                    continue
                if partition_hour >= now_local.replace(minute=0, second=0, microsecond=0):
                    continue
                try:
        ...
        with self._lock:
            ...
            for snapshot in clean_snapshots:
                ...
                if snapshot_id in known:
                    ...
                    continue
                directory = self._hour_directory(self.root, snapshot)
