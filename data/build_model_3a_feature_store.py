"""
build_model_3a_feature_store.py

Build Model 3A feature store = Model 2A v2 feature store + 5 trend-relation features.
Reads existing 2A store, computes new features, appends columns, saves.

Output: data/model_3a_feature_store.parquet
"""
...
OUTPUT_PATH = Path("data/model_3a_feature_store.parquet")
...
def main():
    print("=" * 60)
    print("  Model 3A Feature Store Builder")
    print("=" * 60)
    ...
    # Quick sanity
    for c in ["temp_direction_alignment", "temp_short_long_ratio",
              "temp_volatility_ratio_60m_360m", "temp_reversal_count_120m",
              "temp_direction_persistence_60m"]:
        if c in df.columns:
            v = df[c]
            print(f"  {c}: mean={v.mean():.3f}, std={v.std():.3f}, min={v.min():.3f}, max={v.max():.3f}")
        else:
            print(f"  {c}: MISSING!")
