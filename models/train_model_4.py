"""
Train Model 4 — Model 3B (59 features) + HKO forward-looking forecast features.

LightGBM quantile regression (remaining_upside) + binary classifier (is_upside_zero).
Two variants:
  Variant 1 (full):    Uses full history (same split as Model 3B).
  Variant 2 (restricted): Uses rows with target_date >= 2023-06-01 only.

New features (8 total):
  - forecast_rain_prob_morning   — morning rain level from segment parser (0.4×weight)
  - forecast_rain_prob_afternoon — afternoon rain level from segment parser (1.0×weight)
  - forecast_rain_prob_overall   — worst-case rain level across all segments
  - forecast_rain_prob_missing   — 1 if no weather description available
  - forecast_rain_prob_label     — HKO probability label ordinal (低→1 … 高→5, 0=none)
  - forecast_min_rh              — forecast min relative humidity
  - forecast_max_rh              — forecast max relative humidity
  - forecast_rh_range            — forecast_max_rh - forecast_min_rh

Outputs:
  models/intraday_minute_ai_model_4/          (Variant 1)
  models/intraday_minute_ai_model_4_restricted/ (Variant 2)
"""
...
FEATURE_STORE_PATH = Path("data/model_4_feature_store.parquet")
MODEL_DIR_FULL = Path("models/intraday_minute_ai_model_4")
MODEL_DIR_RESTRICTED = Path("models/intraday_minute_ai_model_4_restricted")
...
TREND_RELATION_FEATURES = [
    "temp_direction_alignment",
    "temp_short_long_ratio",
    "temp_volatility_ratio_60m_360m",
    "temp_reversal_count_120m",
    "temp_direction_persistence_60m",
]

# ── Model 4 new features ─────────────────────────────────────────────

FORECAST_RAIN_FEATURES = [
    "forecast_rain_prob_morning",
    "forecast_rain_prob_afternoon",
    "forecast_rain_prob_overall",
    "forecast_rain_prob_missing",
    "forecast_rain_prob_label",
    "forecast_min_rh",
    "forecast_max_rh",
    "forecast_rh_range",
]

FEATURE_COLS = BASE_FEATURES + RAIN_FEATURES + TREND_RELATION_FEATURES + FORECAST_RAIN_FEATURES
...
def load_and_prepare():
    logger.info("Loading Model 4 feature store...")
...
def main():
    ...
    logger.info("=== Training Model 4 full variant ===")
    df_oot_full, eval_full, rain_full = train_variant(
        df, MODEL_DIR_FULL, "Model 4 full", restrict=False
    )

    # Variant 2: restricted to >= 2023-06-01
    logger.info("=== Training Model 4 restricted variant ===")
    df_oot_restricted, eval_restricted, rain_restricted = train_variant(
        df, MODEL_DIR_RESTRICTED, "Model 4 restricted", restrict=True
    )

    total_features = len(BASE_FEATURES) + len(RAIN_FEATURES) + len(TREND_RELATION_FEATURES)
    new_features = len(FORECAST_RAIN_FEATURES)

    logger.info(f"\n{'=' * 60}")
    logger.info("Training complete!")
    logger.info(f"  Full model: {MODEL_DIR_FULL}")
    logger.info(f"  Restricted model: {MODEL_DIR_RESTRICTED}")
    logger.info(f"  Features: {len(FEATURE_COLS)} ({total_features} base + "
                f"{new_features} forecast-rain)")
