"""
Train Model 3B — Model 2B + 5 trend-relation features.

LightGBM quantile regression (remaining_upside) + binary classifier (is_upside_zero).
Two variants:
  Variant 1 (full):    Uses full history from Model 3A.
  Variant 2 (restricted): Uses rows with target_date >= 2023-06-01 only.

Outputs:
  models/intraday_minute_ai_model_3b/          (Variant 1)
  models/intraday_minute_ai_model_3b_restricted/ (Variant 2)

The 5 extra features compare short-term temperature changes against long-term
background trends, helping the model distinguish temporary noise (passing clouds)
from sustained directional moves — especially on rainy days when temperature stalls.
"""
...
FEATURE_STORE_PATH = Path("data/model_3b_feature_store.parquet")
MODEL_DIR_FULL = Path("models/intraday_minute_ai_model_3b")
MODEL_DIR_RESTRICTED = Path("models/intraday_minute_ai_model_3b_restricted")
...
# 5 trend-relation features distinguishing noise from sustained trends
...
FEATURE_COLS = BASE_FEATURES + RAIN_FEATURES + TREND_RELATION_FEATURES
...
def load_and_prepare():
    logger.info("Loading Model 3B feature store...")
...
def main():
    ...
    df_oot_full, eval_full, rain_full = train_variant(
        df, MODEL_DIR_FULL, "Model 3B full", restrict=False
    )

    # Variant 2: restricted to >= 2023-06-01
    df_oot_restricted, eval_restricted, rain_restricted = train_variant(
        df, MODEL_DIR_RESTRICTED, "Model 3B restricted", restrict=True
    )
    ...
    logger.info(f"  Features: {len(FEATURE_COLS)} ({len(BASE_FEATURES)} base + "
                f"{len(RAIN_FEATURES)} rain + {len(TREND_RELATION_FEATURES)} trend)")
