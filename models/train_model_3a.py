"""
Train Model 3A — Model 2A v2 + 5 trend-relation features.

LightGBM quantile regression (remaining_upside) + binary classifier (is_upside_zero).
Output: models/intraday_minute_ml_model_3a/{upside_q*.txt, upside_zero.txt, ...}
...
from sustained directional moves.
"""
...
FEATURE_STORE_PATH = Path("data/model_3a_feature_store.parquet")
MODEL_DIR = Path("models/intraday_minute_ml_model_3a")
...
MODEL_2A_FEATURES = [
    ...
    # wind (8) - offshore_highland variant
...
FEATURE_COLS = MODEL_2A_FEATURES + TREND_RELATION_FEATURES
...
def load_and_prepare():
    logger.info("Loading Model 3A feature store...")
...
def main():
    ...
    logger.info(f"🎉 Model 3A training complete! Features: {len(FEATURE_COLS)}"
                f" ({len(MODEL_2A_FEATURES)} base + {len(TREND_RELATION_FEATURES)} trend)")
